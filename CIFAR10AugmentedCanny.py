...
if __name__ == "__main__":
    ...
    for i in range(X_train.shape[0]):
        for j in range(X_train.shape[3]):
            blur = cv2.GaussianBlur(X_train_aug[i, :, :, j], (3, 3), 0)
            X_train[i, :, :, j] = np.where(cv2.Canny(blur, 100, 200) != 0, 1, 0)

    for i in range(X_test.shape[0]):
        for j in range(X_test.shape[3]):
            blur = cv2.GaussianBlur(X_test_org[i, :, :, j], (3, 3), 0)
            X_test[i, :, :, j] = np.where(cv2.Canny(blur, 100, 200) != 0, 1, 0)
    ...
    for epoch in range(args.epochs):
        ...
        print(
            "#%d Augmented Canny Accuracy: %.2f%% Training: %.2fs Testing: %.2fs"
            % (
                epoch + 1,
                result_test,
        ...
        np.savetxt(
            "class_sums/CIFAR10AugmentedCanny_%d_%d_%d_%.1f_%d_%d_%d.txt"
            % (
                epoch + 1,
                args.num_clauses,
                args.T,
